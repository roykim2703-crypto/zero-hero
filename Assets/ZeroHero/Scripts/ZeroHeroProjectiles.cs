...
namespace ZeroHero
{
    public sealed partial class ZeroHeroGame
    {
        ...
        void AddHazard(Vector2 center,Vector2 size,float warning,float life,int damage,Color color,bool stone=false)
        {
            var sr=Sprite("Attack warning",art.square,new Color(color.r,color.g,color.b,.18f),center,size,12);
            hazards.Add(new Hazard { sprite=sr,pos=center,size=size,warning=warning,life=life,damage=damage,color=color,stone=stone });
        }
